...
export const POST = async (req: NextRequest) => {
    ...
    try {
        ...
        return NextResponse.json({ message: "Error locating staff member", error }, { status: 500 });
    }
    ...
    try {
        ...
        await prisma.bookHistory.create({
            data: {
                ...
                ...(selectedUserType === "studentUser" && { Student: { connect: { id: data.user.id } } }),
            },
        });

        // Return success response
        return NextResponse.json({
            message: `Successfully loaned out a book to ${selectedUserType === "studentUser" ? "student" : "staff"} ${data.user.firstName} ${data.user.lastName}.`,
        });
    } catch (error) {
