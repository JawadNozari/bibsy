...
export default function RegisterMember() {
	...
	const handleSubmit = async (e: React.SyntheticEvent) => {
		...
		axios
			...
			.catch((error: Error) => {
				...
				setAlertMessage("Unable to register user. This user may already exist");
				setShowMessage(true);
				setTimeout(() => {
					setShowMessage(false);
				}, 3000);
	...
	const generateClassroomOptions = () => {
		...
		classroomOptions.sort((a, b) => {
			const numA = Number.parseInt(a.slice(0, 2));
			const numB = Number.parseInt(b.slice(0, 2));
			return numA - numB;
		});
	...
	return (
		<Suspense fallback={<div />}>
										...
										>
											<label htmlFor="students">
												<input
													type="radio"
													value="students"
													onChange={() => handleRadioChange("students")}
													checked={selectedValue === "students"}
												/>
												Students
											</label>
											<br />
			...
			<div className="fixed bottom-10 right-10">
				{showMessage && <Alert alertType={alertType} message={alertMessage} />}
			</div>
		</Suspense>
	);
}
